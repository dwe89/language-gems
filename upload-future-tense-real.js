...
async function uploadFutureTenseContent() {
  try {
    // Read the content file
    const contentData = JSON.parse(fs.readFileSync('future-tense-real-content.json', 'utf8'));
    
    // Get the future topic ID
    ...
    if (topicError || !topic) {
      console.error('Error finding future topic:', topicError);
      return;
    }
    
    console.log('Found future topic:', topic.id);
    ...
    console.log('🎉 Spanish Future Tense content uploaded successfully!');
