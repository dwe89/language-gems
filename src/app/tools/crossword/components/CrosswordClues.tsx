...
export function CrosswordClues({ 
  ...
  const renderClueSection = (title: string, clues: CrosswordClue[], direction: 'across' | 'down') => {
    ...
    return (
      <div className="mb-8">
        <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4 pb-2 border-b-2 border-blue-500">
          {title}
        </h3>
          ...
          {clues.map((clue) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'flex items-start p-2 rounded-md cursor-pointer transition-all duration-200',
                  {
                    'bg-blue-100 dark:bg-blue-900/30': isSelected,
                    'bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700': !isSelected && onClueClick,
                    'cursor-default bg-slate-50 dark:bg-slate-700/50': !onClueClick,
                  }
                )}
                onClick={() => handleClueClick(clue.number, direction)}
              >
                <span className="font-bold text-base text-slate-900 dark:text-slate-100 min-w-[2rem]">
                  {clue.number}.
                </span>
                <div className="flex-1 ml-2">
                  ...
                  {showAnswers && (
                    <div className="mt-2">
                      <span className="inline-flex items-center px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded font-mono font-bold text-xs">
                        {clue.answer}
                      </span>
                    </div>
                  )}
