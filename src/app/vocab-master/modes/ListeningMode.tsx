  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <motion.div
        ...
        className={`rounded-2xl p-4 ${isAdventureMode
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            {onExit && (
              <button
                onClick={onExit}
                className={`flex items-center gap-1 px-2 py-1 rounded-lg text-sm font-medium ${isAdventureMode
            ...
            <div className={`p-2 rounded-full ${isAdventureMode ? 'bg-purple-500/20' : 'bg-purple-100'
              }`}>
              <Ear className={`h-5 w-5 ${isAdventureMode ? 'text-purple-300' : 'text-purple-600'
                }`} />
            </div>
            <div>
              <h3 className={`font-bold ${isAdventureMode ? 'text-white' : 'text-gray-800'
                }`}>
          ...
          <div className="flex items-center space-x-6">
            <div className="text-center">
              <div className={`text-2xl font-bold ${isAdventureMode ? 'text-green-400' : 'text-green-600'
                }`}>
                {gameState.correctAnswers}
              </div>
              <div className={`text-xs ${isAdventureMode ? 'text-slate-400' : 'text-gray-500'
                }`}>
                Correct
              </div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${isAdventureMode ? 'text-red-400' : 'text-red-600'
                }`}>
            ...
            <div className="text-center">
              <div className={`text-2xl font-bold ${isAdventureMode ? 'text-blue-400' : 'text-blue-600'
                }`}>
                {accuracy}%
              </div>
              <div className={`text-xs ${isAdventureMode ? 'text-slate-400' : 'text-gray-500'
        ...
        {/* Progress Bar */}
        <div className="mt-4">
          <div className={`h-2 rounded-full overflow-hidden ${isAdventureMode ? 'bg-slate-700' : 'bg-gray-200'
            }`}>
            <motion.div
      ...
      <motion.div
        ...
        className={`rounded-3xl p-8 text-center ${isAdventureMode
      ...
      >
        ...
        <div className="mb-8">
          <motion.div
            ...
            className={`w-32 h-32 mx-auto rounded-full flex items-center justify-center relative ${isAdventureMode
          ...
          >
            <Headphones className={`h-16 w-16 ${isAdventureMode ? 'text-purple-300' : 'text-purple-600'
              }`} />
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex space-x-1">
                {audioWaves.map((height, index) => (
                  <motion.div
                    key={index}
                    className={`w-1 rounded-full ${isAdventureMode ? 'bg-purple-400/60' : 'bg-purple-500/60'
                      }`}
                    animate={{
        ...
        <h2 className={`text-3xl font-bold mb-4 ${isAdventureMode ? 'text-white' : 'text-gray-800'
          }`}>
          Listen & Translate
        </h2>

        <p className={`text-lg mb-8 ${isAdventureMode ? 'text-slate-300' : 'text-gray-600'
          }`}>
        ...
        <div className="flex justify-center items-center space-x-4 mb-8">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`p-4 rounded-full transition-all duration-200 ${gameState.audioPlaying || isPlaying
                ? isAdventureMode
      ...
      <motion.div
        ...
        className={`rounded-2xl p-6 ${isAdventureMode
      ...
      >
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className={`text-xl font-semibold ${isAdventureMode ? 'text-white' : 'text-gray-800'
              }`}>
              English Translation
            </h3>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-bold ${isAdventureMode
          ...
          <div className="relative">
            <input
              ...
              className={`w-full p-4 text-xl font-medium rounded-xl transition-all duration-200 ${isAdventureMode
            ...
            {/* Input Enhancement */}
            <div className={`absolute inset-0 rounded-xl pointer-events-none ${userAnswer.trim()
            ...
            {/* Character Count */}
            {userAnswer.length > 0 && (
              <div className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-sm ${isAdventureMode ? 'text-slate-400' : 'text-gray-400'
                }`}>
                {userAnswer.length}
              </div>
            )}
          ...
          <motion.button
            ...
            className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-200 ${userAnswer.trim()
                ? isAdventureMode
