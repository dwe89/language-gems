...
export default function LavaTempleAnimation({ mistakes, maxMistakes, isMobile = false }: LavaTempleAnimationProps) {
  ...
  return (
    <div
      className={`relative w-full ${isMobile ? 'h-64' : 'h-96 md:h-[28rem] lg:h-[36rem]'} mb-4 overflow-hidden rounded-xl bg-slate-900`}
    >
      ...
      {/* Mystery progress display - left side - Hidden on Mobile */}
      {!isMobile && (
      ...
      {/* Warning stones on the right side - Hidden on Mobile */}
      {!isMobile && (
      ...
      {/* Alert messages */}
      {mistakeRatio > 0.8 && (
        <div className={`absolute ${isMobile ? 'bottom-2 text-sm text-center' : 'bottom-10 text-lg'} left-1/2 transform -translate-x-1/2 text-red-500 font-bold animate-pulse`}>
