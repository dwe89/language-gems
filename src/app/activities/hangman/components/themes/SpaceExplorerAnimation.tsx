...
export default function SpaceExplorerAnimation({
  ...
  const [showWarningPopup, setShowWarningAlert] = useState(false);
  ...
  useEffect(() => {
    ...
    if (mistakes > lastMistakeCount) {
      ...
      if (mistakes >= 4) {
        ...
        setTimeout(() => setShowWarningAlert(false), 4000);
      }
  ...
  return (
    <div className={`relative w-full ${isMobile ? 'h-64' : 'h-96 md:h-[28rem] lg:h-[36rem]'} mb-4 overflow-hidden rounded-xl bg-slate-900`}>
      ...
      <div
        ...
        style={{
          ...
          width: `${isMobile ? astronautPosition.size * 0.8 : astronautPosition.size}px`,
          height: `${isMobile ? astronautPosition.size * 0.8 : astronautPosition.size}px`,
      ...
      {/* Status overlay with ship information - Hidden on mobile */}
      {!isMobile && (
      ...
      {/* Distance calculator tooltip - Hidden on mobile */}
      {!isMobile && (
      ...
      {/* Oxygen Alert Popup - Adjusted for mobile position */}
      <AnimatePresence>
        {showOxygenAlert && (
          <motion.div
            className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/70 border-2 border-red-500 rounded-lg p-4 text-center ${isMobile ? 'max-w-[200px] p-2' : 'max-w-xs'}`}
            ...
            style={{ zIndex: 10 }}
          >
            <div className={`text-red-500 ${isMobile ? 'text-sm' : 'text-lg'} font-bold mb-2`}>OXYGEN ALERT</div>
            <div className={`text-cyan-300 ${isMobile ? 'text-xs' : 'text-sm'}`}>
      ...
      {/* Warning Popup for Critical Levels - Adjusted for mobile position */}
      <AnimatePresence>
        {showWarningPopup && mistakes >= 4 && (
          <motion.div
            className={`absolute ${isMobile ? 'top-12' : 'top-24'} left-1/2 transform -translate-x-1/2 bg-red-900/70 border border-red-500 rounded-lg p-3 text-center`}
          ...
          >
            <motion.div
              className={`text-red-300 ${isMobile ? 'text-[10px]' : 'text-xs'} font-bold`}
      ...
      {mistakeRatio > 0.8 && (
        <motion.div
          className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-red-500 font-bold z-50 ${isMobile ? 'text-2xl text-center' : 'text-6xl'}`}
          animate={{ opacity: [1, 0.5, 1] }}
          transition={{ duration: 0.5, repeat: Infinity }}
        >
          {isMobile ? "CRITICAL\nFAILURE" : "CRITICAL SYSTEMS FAILURE"}
        </motion.div>
      )}
