...
export default function TokyoNightsAnimation({
  ...
  return (
    <div className={`relative w-full ${isMobile ? 'h-64' : 'h-96 md:h-[28rem] lg:h-[36rem]'} mb-4 overflow-hidden rounded-xl bg-slate-900`}>
      ...
      {/* Video background - adjusted for mobile height */}
      <div className="absolute inset-0 overflow-hidden">
        <video
          className={`absolute inset-0 w-full h-full object-cover ${isMobile ? 'scale-125 translate-y-4' : 'scale-110 md:scale-100 translate-y-16 md:translate-y-8'}`}
          autoPlay
          muted
          loop
          playsInline
      ...
      {securityState.faceDetection && (
        <div className={`absolute ${isMobile ? 'top-1/4 scale-75' : 'top-1/4'} left-1/2 transform -translate-x-1/2 w-32 h-32`}>
      ...
      {/* Terminal information overlay - Hid on Mobile via strict check */}
      {!isMobile && (
      ...
      {/* Warning message - Hid on Mobile via strict check */}
      {!isMobile && (
      ...
      {mistakeRatio > 0.7 && (
        <div className={`absolute top-6 left-1/2 transform -translate-x-1/2 bg-black/70 border border-red-500 rounded px-3 py-1 font-mono text-xs z-50 ${isMobile ? 'scale-75' : ''}`}>
          <motion.div
      ...
      {mistakeRatio > 0.8 && (
        <motion.div
          className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-red-500 font-bold font-mono z-50 ${isMobile ? 'text-4xl text-center' : 'text-6xl'}`}
          animate={{ opacity: [1, 0.3, 1] }}
          transition={{ duration: 0.5, repeat: Infinity }}
        >
          {isMobile ? "SYSTEM\nOVERRIDE" : "SYSTEM OVERRIDE IMMINENT"}
        </motion.div>
      )}
    </div>
  );
