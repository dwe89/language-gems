...
function DefaultEngine({
  ...
  const updateObjects = useCallback(() => {
    ...
    // Check if correct answer has fallen off screen BEFORE filtering
    // This fixes the bug where missing gems causes the game to stop spawning
    const hasMissedCorrectAnswer = physicsObjectsRef.current.some(
      obj => obj.y >= (screenHeight + 100) && obj.isCorrect
    );

    if (hasMissedCorrectAnswer) {
      console.log('❌ Correct answer missed (fell off screen)');
      onIncorrectAnswer();
      setVocabObjects([]); // Clear objects immediately
