...
export default function TicTacToeGame({
  ...
  const makeComputerMove = (currentBoard = board) => {
    ...
    if (settings.difficulty === 'advanced') {
      ...
      // 75% strategic moves (reduced from 85%), 25% random for some unpredictability
      if (Math.random() < 0.75) {
      ...
      // Beginner: 40% strategic moves (reduced from 60%), 60% random
      if (Math.random() < 0.40) {
        const strategicMove = getStrategicMove(currentBoard);
        bestMove = strategicMove ?? availableCells[Math.floor(Math.random() * availableCells.length)];
      } else {
  ...
  const handleGameEnd = async (gameWinner: 'X' | 'O' | 'tie', line: number[]) => {
    if (gameWinner === 'tie') {
      setGameState('tie');
      setWinner(null);
      playSFX('tie-game');
    ...
    if (isAssignmentMode && assignmentId && userId) {
      ...
      if (exposedWordIds.length > 0) {
        ...
        // AWAIT the recording so parent can check completion after
        try {
          const result = await assignmentExposureService.recordWordExposures(
            assignmentId,
            userId,
            exposedWordIds
          );
          ...
          console.error('❌ [LAYER 2] Exception recording exposures:', err);
        }
      }
    }

    // Call parent callback with enhanced results after a brief delay for animations
    ...
    // Pass per-game stats (wordsLearned, correctAnswers, totalQuestions) to onGameEnd
    // The parent wrapper accumulates these, so we should NOT pass cumulative values
    onGameEnd({
      outcome: gameWinner === 'X' ? 'win' : gameWinner === 'tie' ? 'tie' : 'loss',
      wordsLearned: wordsLearned, // Per-game count, NOT cumulative
      perfectGame,
      correctAnswers: correctAnswers, // Per-game count, NOT cumulative
      totalQuestions: totalQuestions, // Per-game count, NOT cumulative
      timeSpent,
      uniqueWordsThisSession: usedWordsThisSession.size // Total unique words across ALL rounds in this session
    });
  };
                        ...
                        {gameState === 'won' && (
                          ...
                          >
                            <div className="text-2xl font-bold text-green-300 drop-shadow-lg bg-black/50 backdrop-blur-sm rounded-2xl px-4 py-3 border border-green-400/50">
                              <Trophy className="inline w-6 h-6 mr-2" />You Won!
                            </div>
                            <div className="text-sm text-white bg-black/30 backdrop-blur-sm rounded-lg px-3 py-2">
                              Words learned: {wordsLearned}
                            </div>
                            <div className="flex gap-3 justify-center mt-3">
                              <motion.button
                                ...
                                whileTap={{ scale: 0.98 }}
                              >
                                <Target className="inline w-4 h-4 mr-2" />Play Again
                              </motion.button>
                              <motion.button
                                onClick={() => {
                                  playSFX('button-click');
                        ...
                        {gameState === 'lost' && (
                          ...
                          >
                            <div className="text-2xl font-bold text-red-300 drop-shadow-lg bg-black/50 backdrop-blur-sm rounded-2xl px-4 py-3 border border-red-400/50">
                              <Frown className="inline w-6 h-6 mr-2" />Game Over
                            </div>
                            <div className="text-sm text-white bg-black/30 backdrop-blur-sm rounded-lg px-3 py-2">
                              Words learned: {wordsLearned}
                            </div>
                    ...
                    <AnimatePresence mode="wait">
                      ...
                      {gameState === 'won' && (
                        <motion.div
                          ...
                          className="text-center"
                        >
                          <div className="text-lg font-bold text-green-300 drop-shadow-lg bg-black/50 backdrop-blur-sm rounded-xl px-3 py-2 border border-green-400/50">
                            <Trophy className="inline w-5 h-5 mr-2" />You Won!
                          </div>
                        </motion.div>
                      )}
                      ...
                      {gameState === 'lost' && (
                        <motion.div
                          ...
                          className="text-center"
                        >
                          <div className="text-lg font-bold text-red-300 drop-shadow-lg bg-black/50 backdrop-blur-sm rounded-xl px-3 py-2 border border-red-400/50">
                            <Frown className="inline w-5 h-5 mr-2" />Game Over
                          </div>
                        </motion.div>
                      )}
