...
/**
 * POST /api/admin/aqa-reading/update
 * Update an existing AQA reading paper and its questions
 * Admin only endpoint
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (paper) {
      ...
      const timeLimit = paper.tier ? (paper.tier === 'foundation' ? 45 : 60) : existingPaper.time_limit_minutes;
    ...
    console.error('Error in POST /api/admin/aqa-reading/update:', error);
    return NextResponse.json(
