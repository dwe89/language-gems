...
/**
 * POST /api/admin/aqa-writing/create
 * Create a new AQA writing paper with questions
 * Admin only endpoint
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const timeLimit = paper.tier === 'foundation' ? 70 : 75;
    const totalMarks = 50; // Always 50 marks

    // Auto-generate description if not provided
    const description = paper.description || 
      `AQA GCSE ${paper.language.toUpperCase()} Writing Assessment - ${paper.tier === 'foundation' ? 'Foundation' : 'Higher'} Tier`;
    ...
    console.error('Error in POST /api/admin/aqa-writing/create:', error);
    return NextResponse.json(
