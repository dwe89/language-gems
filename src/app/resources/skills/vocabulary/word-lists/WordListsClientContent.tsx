...
export default function WordListsClientContent() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/resources/skills?category=word-list');
        if (response.ok) {
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl p-12 border border-slate-200 shadow-sm text-center mb-8">
          ...
          <h3 className="text-xl font-bold text-slate-800 mb-2">No Word Lists Found</h3>
          <p className="text-slate-600 mb-6">
            There are no word lists available yet.
          </p>
          <p className="text-sm text-slate-500">
            Add new word lists via the admin dashboard.
          </p>
        </div>
      ) : (
