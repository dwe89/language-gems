...
export default function GamesDevWarningDialog() {
  const [open, setOpen] = useState(false);

  useEffect(() => {
    try {
      const seen = localStorage.getItem('games-dev-warning-seen');
  ...
  const dismiss = () => {
    try {
      localStorage.setItem('games-dev-warning-seen', 'true');
    } catch (e) {}
    setOpen(false);
  };
      ...
      <DialogContent className="sm:max-w-xl bg-slate-900/95 border-0 p-6 sm:p-8">
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-2xl font-semibold text-white">Games - Work in Progress</h3>
              <p className="mt-2 text-sm text-gray-300 max-w-xl">Not all games have been fully developed and some features may not work as expected. Thanks for your patience as we build this section.</p>
            </div>
          </div>
          <div>
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="text-amber-900">
              ...
              <ul className="list-disc list-inside space-y-1 text-sm">
                <li>Some games may be missing or incomplete</li>
                <li>Game mechanics and difficulty levels are still being balanced</li>
                <li>Progress tracking and rewards systems are evolving</li>
                <li>Content and gameplay may change without notice</li>
