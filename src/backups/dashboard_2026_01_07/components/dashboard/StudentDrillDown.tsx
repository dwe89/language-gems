...
import type { StudentProfileData, TimeRange } from '@/types/teacherAnalytics';
  ...
  const fetchStudentProfile = async () => {
    try {
      ...
      const response = await fetch(
        `/api/teacher-analytics/student-profile?studentId=${studentId}&timeRange=${timeRange}`
      );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <div className="text-xs text-gray-500 mt-1">
            {TIME_RANGE_OPTIONS.find(o => o.value === timeRange)?.label || 'Selected period'}
          </div>
        </div>
