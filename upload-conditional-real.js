...
async function uploadConditionalContent() {
  try {
    // Read the content file
    const contentData = JSON.parse(fs.readFileSync('conditional-real-content.json', 'utf8'));
    
    // Get the conditional topic ID
    ...
    if (topicError || !topic) {
      console.error('Error finding conditional topic:', topicError);
      return;
    }
    
    console.log('Found conditional topic:', topic.id);
    ...
    const practiceContent = {
      ...
      title: 'Spanish Conditional Practice',
      slug: 'conditional-practice',
    ...
    const quizContent = {
      topic_id: topic.id,
      content_type: 'quiz',
      title: 'Spanish Conditional Quiz',
    ...
    console.log('🎉 Spanish Conditional content uploaded successfully!');
