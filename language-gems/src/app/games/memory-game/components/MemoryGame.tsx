    ...
    >
      ...
      <div className="game-board bg-white shadow-lg rounded-xl p-4 max-w-6xl mx-auto">
        ...
        {gameOver ? (
          ...
          <div className={`cards-grid ${getGridColumns()} gap-3 md:gap-4`}>
            {cards.map(card => (
              <motion.div
                key={card.id}
                className={`memory-card ${flippedCards.includes(card.id) || matchedCards.includes(card.id) ? 'flipped' : ''} ${matchedCards.includes(card.id) ? 'matched' : ''}`}
              ...
              >
                <div className="memory-card-inner">
                  <div className={`memory-card-front ${themeStyle.primary}`}>
                    <div className="text-white text-3xl">?</div>
                  </div>
                  <div className={`memory-card-back ${themeStyle.secondary}`}>
                    {card.term.startsWith('http') || card.term.startsWith('data:') ? (
                      <img 
                        src={card.term} 
                        alt="Card image" 
                      ...
                      <span className={`${themeStyle.text} text-lg`}>{card.term}</span>
                    )}
                  </div>
                </div>
              </motion.div>
