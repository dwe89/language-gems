...
async function handleCreateAQAReadingAssignment(args) {
    const { title, teacher_id, class_id, student_id, assessment_level, due_date, custom_time_limit, custom_instructions } = args;
...
async function handleGetAQAReadingResults(args) {
...
async function handleGetAQAReadingAnalytics(args) {
...
function processAnalyticsByQuestionType(results) {
    const typeStats = {};
    results?.forEach(result => {
        Object.entries(result.performance_by_question_type || {}).forEach(([type, stats]) => {
            if (!typeStats[type]) {
                typeStats[type] = {
                    total_questions: 0,
                    total_correct: 0,
...
function processAnalyticsByTheme(results) {
    const themeStats = {};
    results?.forEach(result => {
        Object.entries(result.performance_by_theme || {}).forEach(([theme, stats]) => {
            if (!themeStats[theme]) {
                themeStats[theme] = {
                    total_questions: 0,
                    total_correct: 0,
...
function processAnalyticsByTopic(results) {
    const topicStats = {};
    results?.forEach(result => {
        Object.entries(result.performance_by_topic || {}).forEach(([topic, stats]) => {
            if (!topicStats[topic]) {
                topicStats[topic] = {
                    total_questions: 0,
                    total_correct: 0,
...
function processAnalyticsByStudent(results) {
    const studentStats = {};
    results?.forEach(result => {
...
async function handleCreateReadingComprehensionTask(args) {
    const { title, language, curriculum_level, exam_board, theme_topic, category, subcategory, difficulty, content, word_count, estimated_reading_time, questions, created_by } = args;
    ...
    if (taskError)
        throw taskError;
    // Create questions
    const questionsWithTaskId = questions.map((q, index) => ({
        task_id: task.id,
...
async function handleGetReadingComprehensionTasks(args) {
    let query = supabase
        .from('reading_comprehension_tasks')
        .select(`
      *,
    ...
    if (args.language)
        query = query.eq('language', args.language);
    if (args.curriculum_level)
        query = query.eq('curriculum_level', args.curriculum_level);
    if (args.exam_board)
        query = query.eq('exam_board', args.exam_board);
    if (args.category)
        query = query.eq('category', args.category);
    if (args.subcategory)
        query = query.eq('subcategory', args.subcategory);
    if (args.difficulty)
        query = query.eq('difficulty', args.difficulty);
    if (args.theme_topic)
        query = query.eq('theme_topic', args.theme_topic);
    // Order and limit
    if (args.random) {
...
async function handleSaveReadingComprehensionResult(args) {
    const { task_id, student_id, assignment_id, total_questions, correct_answers, score_percentage, time_spent_seconds, passed, question_results } = args;
    ...
    const questionResultsWithId = question_results.map((qr) => ({
        result_id: result.id,
...
async function handleGetReadingComprehensionAnalytics(args) {
    const { student_id, class_id, teacher_id, language, difficulty, date_from, date_to, group_by = 'student' } = args;
    let query = supabase
        .from('reading_comprehension_results')
        .select(`
      *,
    ...
    if (student_id)
        query = query.eq('student_id', student_id);
    if (language)
        query = query.eq('reading_comprehension_tasks.language', language);
    if (difficulty)
        query = query.eq('reading_comprehension_tasks.difficulty', difficulty);
    if (date_from)
...
function processStudentAnalytics(results) {
...
function processDifficultyAnalytics(results) {
...
function processCategoryAnalytics(results) {
...
function processLanguageAnalytics(results) {
